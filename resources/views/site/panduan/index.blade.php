...
<head>
    ...
    <meta name="description" content="Semua panduan dan tutorial IT dari pahamIT - step-by-step untuk networking, server, Linux, dan keamanan siber.">
    <title>Panduan Belajar IT - pahamIT</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        body { margin: 0; font-family: "Instrument Sans", ui-sans-serif, system-ui, sans-serif; color: var(--text); background: var(--bg); line-height: 1.65; overflow-x: hidden; transition: background .22s, color .22s; }
        ...
        .page-hero-label { display: inline-flex; align-items: center; gap: 8px; font-size: .72rem; font-weight: 800; letter-spacing: .08em; text-transform: uppercase; color: #93c5fd; background: rgba(11,111,238,.2); border: 1px solid rgba(11,111,238,.3); padding: 4px 12px; border-radius: 999px; margin-bottom: 16px; }
        ...
        /* GUIDE CARD */
        .guide-card { background: var(--surface); border: 1px solid var(--line); border-radius: var(--radius); padding: 22px; box-shadow: var(--shadow); transition: box-shadow .2s, transform .2s, border-color .2s; display: flex; flex-direction: column; gap: 10px; position: relative; overflow: hidden; }
        .guide-card::before { content: ""; position: absolute; top: 0; left: 0; right: 0; height: 3px; background: linear-gradient(90deg, var(--blue), var(--red)); opacity: 0; transition: opacity .2s; }
        .guide-card:hover { box-shadow: var(--shadow-md); transform: translateY(-3px); border-color: rgba(11,111,238,.2); }
        .guide-card:hover::before { opacity: 1; }
        .guide-num { font-size: 2rem; font-weight: 900; color: rgba(11,111,238,.15); line-height: 1; font-variant-numeric: tabular-nums; }
        .guide-cat { font-size: .68rem; font-weight: 800; letter-spacing: .08em; text-transform: uppercase; color: var(--blue); }
        .guide-title { font-size: .95rem; font-weight: 800; line-height: 1.35; color: var(--text); transition: color .15s; }
        .guide-card:hover .guide-title { color: var(--blue); }
        .guide-excerpt { font-size: .82rem; color: var(--muted); line-height: 1.6; flex: 1; }
        .guide-meta { display: flex; align-items: center; gap: 10px; font-size: .72rem; color: var(--muted); font-weight: 600; margin-top: 4px; }
        .guide-link { display: inline-flex; align-items: center; gap: 5px; font-size: .78rem; font-weight: 800; color: var(--blue); margin-top: auto; }
...
<header class="topbar">
    <div class="nav-inner">
        ...
        <nav class="nav-links">
            ...
            <a href="{{ route('listing.panduan') }}" class="active">Panduan</a>
...
<div class="page-hero">
    <div class="page-hero-inner">
        ...
        <h1>Semua Panduan & Tutorial</h1>
        <p>Tutorial runtut untuk networking, server, Linux, cyber security - dari dasar sampai siap praktik.</p>
    </div>
</div>

<div class="search-bar">
    <div class="search-inner">
        <form class="search-form" method="GET" action="{{ route('listing.panduan') }}">
            <input type="search" name="q" value="{{ $query }}" placeholder="Cari panduan..." autocomplete="off">
        ...
        <div class="filter-row">
            <a class="filter-tag {{ !$cat ? 'active' : '' }}" href="{{ route('listing.panduan', $query ? ['q'=>$query] : []) }}">Semua</a>
            @foreach($categories as $c)
                <a class="filter-tag {{ $cat === $c ? 'active' : '' }}"
                   href="{{ route('listing.panduan', array_filter(['q'=>$query,'kategori'=>$c])) }}">{{ $c }}</a>
            @endforeach
        </div>
        @endif
        @if($query || $cat)
            <a class="clear-btn" href="{{ route('listing.panduan') }}">
...
<div class="main-wrap">
    <p class="result-count">
        Menampilkan <strong>{{ $posts->total() }}</strong> panduan
    ...
    <div class="posts-grid">
        ...
        <a href="{{ route('post.panduan', $post->slug) }}" class="guide-card">
            ...
            @if($post->category)<div class="guide-cat">{{ $post->category }}</div>@endif
            <div class="guide-title">{{ $post->title }}</div>
            @if($post->excerpt)
            <div class="guide-excerpt">{{ Str::limit($post->excerpt, 90) }}</div>
            @endif
            <div class="guide-meta">
                <span>{{ optional($post->published_at)->diffForHumans() ?? '-' }}</span>
                <span>-</span>
                <span>{{ max(1, (int)(str_word_count(strip_tags($post->content ?? '')) / 200)) }} menit baca</span>
                <span>-</span>
                <span>{{ number_format($post->views_count) }} views</span>
            </div>
            <span class="guide-link">Pelajari <svg width="11" height="11" viewBox="0 0 24 24" fill="none"><path d="M5 12h14m-6-6 6 6-6 6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></span>
        </a>
        @endforeach
    </div>
    ...
    <div class="empty">
        ...
        <h3>Tidak ada panduan ditemukan</h3>
